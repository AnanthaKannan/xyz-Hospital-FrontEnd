...
const ListPatientComp = () => {
  ...
  const {
    data: rowData,
    tc: totalCount,
    loading: pListLoading,
  } = useAppSelector((state) => state.patient.patientList);
  ...
  const onCellClicked = async (event: any) => {
    ...
    if (colDef.field === "edit") {
      ...
      sweetConfirmation(
        () => navigate("/create-patient", { state: data }),
        "Yes, Update it!"
      );
      ...
      sweetConfirmation(
        () => dispatch(deletePatientThunk({ id: data._id })),
        "Yes, delete it!"
      );
    }
  };
