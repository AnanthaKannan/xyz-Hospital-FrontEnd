...
describe("#List Doctor", () => {
  ...
  it("First row values", () => {
    ...
    cy.get('.MuiDataGrid-row--firstVisible > [data-field="id"]').should(
      "exist"
    );
    cy.get('.MuiDataGrid-row--firstVisible > [data-field="name"]').should(
      "exist"
    );
    cy.get('.MuiDataGrid-row--firstVisible > [data-field="specialist"]').should(
      "exist"
    );
    cy.get(
      '.MuiDataGrid-row--firstVisible > [data-field="availableDayConvert"]'
    ).should("exist");
    cy.get(
      '.MuiDataGrid-row--firstVisible > [data-field="availableTimeConvert"]'
    ).should("exist");
    cy.get('.MuiDataGrid-row--firstVisible > [data-field="edit"]').should(
      "exist"
    );
    cy.get('.MuiDataGrid-row--firstVisible > [data-field="delete"]').should(
      "exist"
    );
  });

  it("Delete the doctor", () => {
    cy.intercept("DELETE", `${Cypress.env("apiUrl")}/doctor/*`).as(
      "deleteDoctor"
    );
    cy.intercept("GET", `${Cypress.env("apiUrl")}/doctor?*`).as(
      "getDoctorList1"
