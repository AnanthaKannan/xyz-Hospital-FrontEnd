...
describe("Doctor Create", () => {
  ...
  it("Doctor validation", () => {
    ...
    cy.get("#error-specialist").should(
      "have.text",
      "specialist is a required field"
    );

    cy.get("#error-days").should("have.text", "Please select at least one day");
    cy.get("#error-time-piker").should(
      "have.text",
  ...
  it("Create Doctor with all the fields", () => {
    cy.intercept("POST", `${Cypress.env("apiUrl")}/doctor`).as(
      "postCreateDoctor"
    );
    cy.get("#name").type(faker.name.firstName());
    cy.get("#specialist").type(faker.name.lastName());
    const gender = ["male", "female", "others"];
    cy.get(`#gender > ${common.dropDownCss}`).type(
      `${gender[faker.random.number(2)]}{enter}{enter}`
    );
    cy.get("#phone").type(faker.phone.phoneNumber());
    cy.get("#licenseNo").type(faker.finance.account());
    const tomorrow = `00${new Date(
      new Date().getTime() + 24 * 60 * 60 * 1000
    ).getDate()}`;
    cy.get("#licenseExpiryDate")
      .click()
      .get(`.react-datepicker__day--${tomorrow.slice(tomorrow.length - 3)}`)
      .first()
      .click({ multiple: true });
